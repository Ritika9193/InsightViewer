...
async def scrape_amazon_product(asin):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            product_info['NumberOfReviews'] = "Error"
            print(f"Error while extracting number of reviews: {str(e)}")
...
# async def scrape_number_of_reviews(asin):
#     url = f"https://www.amazon.in/dp/{asin}"
...
#         # Navigate to the reviews page by clicking on 'See all reviews'
#         try:
#             see_all_reviews_link = await page.query_selector('a[data-hook="see-all-reviews-link-foot"]')
#             if see_all_reviews_link:
#                 await see_all_reviews_link.click()
#                 await page.wait_for_timeout(3000)  # Give time for the reviews page to load
#             else:
#                 raise Exception("See all reviews link not found")
#         except Exception as e:
...
#         # Now locate the 'cr-filter-info-section' div and extract the number of reviews
#         try:
#             filter_info_element = await page.query_selector('div[data-hook="cr-filter-info-review-rating-count"]')
            
#             if filter_info_element:
#                 filter_info_text = await filter_info_element.inner_text()

#                 # Extract the number of reviews using simple text manipulation
#                 if 'with reviews' in filter_info_text:
#                     reviews_part = filter_info_text.split('with reviews')[0].strip()
#                     number_of_reviews = ''.join(filter(str.isdigit, reviews_part.split()[-1]))
#                     print(f"Extracted Number of Reviews: {number_of_reviews}")
#                 else:
#                     print("Could not find 'with reviews' in the text.")
...
# # Example usage
# import asyncio
# asin = 'B01MRHQYHV'  # Example ASIN
# reviews_count = asyncio.run(scrape_number_of_reviews(asin))
